...
- **Fix — remove the fallback instead of asking nicely.** `8sync ai` / `8sync .` now launch omp with `--tools` DROPping the two redundant searchers `grep` + `glob` (`STEP0_TOOLS` in `crates/cli/src/models.rs`), so code lookup MUST flow through codegraph (CLI) · codebase-memory-mcp · serena. **Verified on omp 17.2.9 that this is safe: `--tools` filters BUILT-INS only — under `--tools=read,bash,todo` the provider request still carried all 48 `mcp__*` tools plus `engine_*`/`wf_state_*`.** MCP/xdev is orthogonal to the allowlist. `lsp` is deliberately KEPT (zero-friction; serena needs `activate_project` per session).
- **Shell escape closed:** new `deploy::ensure_bash_interceptor` writes `bashInterceptor.patterns` into `~/.omp/agent/config.yml` — omp's rule shape is `{ pattern: <regex>, reason: <string> }` (verified against omp's own `explicitExclusions` schema + the runtime `Blocked by bash pattern: ${match}`). Conservative by design: blocks `\brg\b` and `grep` carrying a recursive flag; single-file / log `grep` stays allowed. Idempotent, never clobbers a user-authored `bashInterceptor:` key. Wired into both `global_pass` and `harness_init`.
