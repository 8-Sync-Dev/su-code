---
name: docker
description: Execute research or experiment code inside isolated Docker containers for safe replication, benchmarks, and untrusted-code runs. Use when the user selects Docker as the execution environment or asks to run code safely, in isolation, or in a sandbox.
allowed-tools: Bash(docker:*)
---

# Docker Sandbox

Ported from `companion-inc/feynman`'s `docker` skill (generic CLI usage — no feynman dependency). Run experiment code inside Docker containers while the agent stays on the host. The container gets the project files, runs the commands, and results sync back.

## When to use

- User selects "Docker Sandbox" as the execution environment in the `replication`/`autoresearch` skills
- Running untrusted code from a paper's/repo's codebase
- Experiments that install packages or modify system state
- Any time the user asks to run research code safely/isolated

## How it works

1. Build or pull an appropriate base image for the code.
...
For iterative experiments (e.g. the `autoresearch` skill's loop), create a named container instead of `--rm`:
...
- GPU access requires Docker configured with the NVIDIA Container Toolkit.
