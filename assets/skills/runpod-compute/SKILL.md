---
name: runpod-compute
description: Provision and manage GPU pods on RunPod for explicitly chosen long-running research experiments. Use when a replication, benchmark, or dataset-heavy research run needs persistent GPU compute with SSH access.
---

# RunPod Compute

Ported from `companion-inc/feynman`'s `runpod-compute` skill (generic CLI usage — no feynman dependency). Use `runpodctl` for persistent GPU pods with SSH access during a specific research run. Do not use this for provider administration outside that run; tie every pod to a replication, benchmark, or dataset-heavy objective.
...
- Large datasets required by a replication or benchmark
- Multi-step work needing SSH access between iterations
