...
- **`/auto` — autonomous engine (gsd-pi-style, on omp core)** — `/auto <goal>` plans then runs to DONE via code-enforced `engine_*` tools (durable slice/task state · verify-with-retry gate · git worktree squash-merge), backed by the `8sync-engine` omp extension. `/auto status|resume`. (`/gs` stays a lighter skill-backed lead protocol; the engine is the strong automation.)
