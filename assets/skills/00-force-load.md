...
- **`/auto` — THE autonomous engine (one command, gsd-pi-style, on omp core)** — `/auto <goal>` plans (with integrated research) then runs to DONE via code-enforced `engine_*` tools (durable slice/task state · verify-with-retry gate · gitleaks-gated commit · run-to-done loop enforced by omp's `session_stop` continuation, capped at 8), backed by the `8sync-engine` omp extension. Isolate a risky slice with omp's `task.isolation` (branch mode `omp/task/<id>`). `/auto status|resume`. The single automation entry — no competing command.
