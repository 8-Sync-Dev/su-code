...
- **`/gs` — one-command team loop** — `/gs <goal>` plan+run · bare `/gs` resume · `/gs auto` treo (L3) · `/gs status|next|stop`. Entry hợp nhất của loop này (plan → delegate role → verify-gate → commit → advance off STATE, tới DoD/blocker). Deploy sẵn `~/.omp/agent/commands/gs.md` + `<repo>/.omp/commands/gs.md`; skill `gs` (on-demand) giải thích protocol.
