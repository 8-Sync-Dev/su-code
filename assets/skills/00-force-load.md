...
- **`/gs` — THE autonomous agent-team engine (one command, native omp extension)** — `/gs <goal>` runs a gated workflow (clarify → plan → plan-review → implement → verify → review → UAT → closeout) via code-enforced `gs_*` tools: durable stage state · per-stage model routing · independent plan-critic & reviewer (different model family, enforced) · direct-argv verify gate · doom-loop guard · git worktree · user UAT lock. `/gs --auto` for autonomous gates; `/gs status|continue`. The single automation entry — no competing command.
