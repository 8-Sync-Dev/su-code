...
4. **KHÔNG bao giờ chỉnh sửa trực tiếp `agents/*.md`** — chỉ append qua format end-of-session (mục 4).
...
Output đúng format dưới — 8sync parse → append vào `agents/*.md`:
...
- `8sync .` = session bắt đầu → đọc `agents/*` + `AGENTS.md` ngay.
