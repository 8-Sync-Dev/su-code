...
- **Fast CLI semantic intel** → `codegraph query|explore|node|callers|callees|impact "<symbol>"` via bash (`codegraph index .` once if `.codegraph/` missing; `explore` = source + call paths in one shot).
